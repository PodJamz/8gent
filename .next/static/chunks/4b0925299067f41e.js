...
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝`,u=(e,t,n=20)=>{let i=Math.round(t/100*n),o="█".repeat(i)+"░".repeat(n-i),r=`${t}%`.padStart(4);return`${e.padEnd(16)} [${o}] ${r}`},h=`
 ┌────────────────────────────────────────────────┐
 │  ⚠️  PERMISSION DENIED                         │
 │                                                │
...
Powered by: Claude (Anthropic)`},v=(e,t)=>{if(t.startsWith("/"))return t;if(".."===t){let t=e.split("/").filter(Boolean);return t.pop(),"/"+t.join("/")}return"~"===t?"/home/openclaw":e+"/"+t},w={help:{name:"help",description:"Show available commands",action:()=>({output:`
╔════════════════════════════════════════════════════════════════════╗
║                      AVAILABLE COMMANDS                            ║
╠════════════════════════════════════════════════════════════════════╣
║                                                                    ║
...
║  ${u("GraphQL",85)}     ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝`})},open:{name:"open",description:"Open an app",usage:"open <app>",action:e=>{let t={design:"/design",projects:"/projects",music:"/music",studio:"/studio",blog:"/blog",evolution:"/story",photos:"/photos",humans:"/humans",story:"/story",prototyping:"/prototyping",settings:"/settings",home:"/"};if(!e[0])return{output:`Usage: open <app>

Available apps:
${Object.keys(t).map(e=>`  → ${e}`).join("\n")}`};let n=e[0].toLowerCase();return t[n]?{output:`Opening ${n}...`,navigate:t[n]}:{output:`open: ${e[0]}: Application not found`}}},neofetch:{name:"neofetch",description:"Display system info",action:(e,t)=>{let n;return{output:(n={os:"8gent v1.0.0",host:"openclaw.io",kernel:"React 18.2.0",uptime:"∞ days",shell:"/bin/zsh",theme:t.theme||"claude",terminal:"OpenClaw Terminal",cpu:"Apple Silicon (simulated)",memory:"∞ GB available"},`
${a}
                        ${n.os}
                        ─────────────────
                        Host: ${n.host}
