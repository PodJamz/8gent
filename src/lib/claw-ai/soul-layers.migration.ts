...
// BEFORE:
// import { CLAW_AI_SYSTEM_PROMPT, THEME_CONTEXTS } from '@/lib/claw-ai/system-prompt';

// AFTER:
// import { assembleSoulPrompt } from '@/lib/claw-ai/soul-layers';
...
const systemPrompt = assembleSoulPrompt({
  ...
  providerContext: useLocalProvider
    ? `\n\n## Provider Context\nRunning on LOCAL inference (${providerSettings.primaryProvider === 'lynkr' ? 'Lynkr tunnel' : 'Ollama direct'}). Responses stay on James's machine with zero cloud cost.`
...
/*
import { getLoadedLayers } from '@/lib/claw-ai/soul-layers';
...
Security guarantee:
  A visitor's request path:
    ...
    4. Tool gating further restricts to 6 portfolio tools
