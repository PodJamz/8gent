/**
 * 8gent Search - System content search
 *
  ...
  for (const skill of DATA.skills) {
    ...
    if (relevance > 0 || normalize(skill).includes(normalizedQuery)) {
      results.push({
        ...
        type: 'skill',
        title: skill,
        description: `8gent has expertise in ${skill}`,
  ...
  for (const theme of themes) {
    ...
    if (relevance > 0) {
      results.push({
        ...
        type: 'theme',
        title: theme.label,
        description: `The ${theme.label} theme for the workspace`,
  ...
  if (grouped.project) {
    sections.push(
      `**Projects:**\n${grouped.project
        .map(
          (r) =>
            `- ${r.title}: ${r.description.slice(0, 150)}...${r.metadata?.technologies
