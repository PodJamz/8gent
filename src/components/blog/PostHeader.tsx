            ...
            <div className="flex items-center gap-3">
              {authors.map((author, index) => (
                <div key={author.name} className="group relative flex items-center gap-3">
                  ...
                  {/* Tooltip */}
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-10 hidden group-hover:block bg-card text-card-foreground text-xs rounded-lg px-3 py-2 shadow-xl whitespace-nowrap border border-border">
                    <div className="font-semibold">{author.name}</div>
                    <div className="text-muted-foreground">{author.role}</div>
                    {/* Arrow */}
                    <div className="absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-card"></div>
                  </div>

                  {/* Separator between multiple authors */}
                  {index < authors.length - 1 && (
                    <div className="w-px h-8 bg-border ml-3"></div>
                  )}
                </div>
              ))}
            </div>
