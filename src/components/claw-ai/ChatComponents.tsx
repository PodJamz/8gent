...
export function WeatherWidget({ data }: { data: WeatherData }) {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-xl p-4 border border-border"
    >
...
export function KanbanTicketCard({ ticket }: { ticket: KanbanTicket }) {
  return (
    <motion.div
      ...
      className="bg-muted/50 rounded-xl p-3 border border-border hover:border-primary/50 transition-colors"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {ticket.description && (
            <p className="text-xs text-muted-foreground/80 mt-1 line-clamp-2">{ticket.description}</p>
          )}
...
export function ProjectCard({ project }: { project: ProjectData }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 rounded-xl p-4 border border-border"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {project.href && (
              <Link href={project.href} className="text-muted-foreground/60 hover:text-foreground">
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {project.technologies.slice(0, 4).map((tech) => (
              <span key={tech} className="px-2 py-0.5 rounded-full text-[10px] bg-muted text-foreground/90">
                {tech}
              </span>
            ))}
          </div>
...
export function WorkExperienceCard({ work }: { work: WorkExperience }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-gradient-to-br from-amber-500/10 to-orange-500/10 rounded-xl p-4 border border-border"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {work.description && (
            <p className="text-xs text-muted-foreground/80 mt-2 line-clamp-2">{work.description}</p>
          )}
        </div>
      </div>
    </motion.div>
    ...
    >
      <Link href={`/design/${theme.name}`}>
        ...
        <div className="p-3 bg-muted/50">
          ...
          <p className="text-xs text-muted-foreground/80 mt-1">Tap to preview theme</p>
        </div>
      </Link>
    </motion.div>
  );
