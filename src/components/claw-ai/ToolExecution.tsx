  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border/50">
        <div className={`p-1.5 rounded-lg ${status === 'complete' ? 'bg-green-500/20' : 'bg-muted'}`}>
          {status === 'executing' ? (
...
function SearchResults({ data }: { data: Record<string, unknown> }) {
  ...
  if (!results || results.length === 0) {
    return <p className="text-xs text-muted-foreground/80">No results found</p>;
  }

  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground/80">Found {results.length} results</p>
      <div className="space-y-1.5 max-h-40 overflow-y-auto">
        {results.slice(0, 5).map((r) => (
          <div key={`${r.type}-${r.title}-${r.url || ''}`} className="flex items-start gap-2 p-2 rounded-lg bg-muted/50">
            <span className="text-[10px] uppercase text-muted-foreground/50 mt-0.5">{r.type}</span>
...
function NavigationResult({
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground">{message || 'Ready to navigate'}</p>
      {url && onNavigate && (
        <button
...
function ScheduleResult({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground">{message || 'Opening calendar...'}</p>
      {url && (
        <a
          href={url}
          target="_blank"
...
function ThemesResult({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground/80">{count} themes available</p>
...
function OpenSearchResult({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground">{message || 'Opening Search...'}</p>
      {query && (
        <p className="text-[10px] text-muted-foreground/60">Query: "{query}"</p>
      )}
    </div>
  );
}
...
function KanbanResult({ data }: { data: Record<string, unknown> }) {
  ...
  if (!tasks || tasks.length === 0) {
    return <p className="text-xs text-muted-foreground/80">No tasks found</p>;
  }
...
function PhotosResult({ data }: { data: Record<string, unknown> }) {
  ...
  if (!photos || photos.length === 0) {
    return <p className="text-xs text-muted-foreground/80">No photos found</p>;
  }

  return <PhotoGallery photos={photos} />;
}

// Render UI result display - uses StreamingUIRenderer with json-render
function RenderUIResult({ data }: { data: Record<string, unknown> }) {
  ...
  const isStreaming = data._streaming as boolean | undefined;

  if (!uiTreeData) {
    return <p className="text-xs text-muted-foreground/80">No UI to render</p>;
  }
  ...
  if (!tree && !isStreaming) {
    return (
      <div className="text-xs text-destructive">
        ...
        <pre className="text-muted-foreground/60 mt-1 overflow-auto text-[10px]">
