...
import { Plus, RotateCcw, Search, Filter, ChevronLeft, ChevronRight, Layers } from 'lucide-react';
import { useProject, type ProjectKanbanTask } from '@/context/ProjectContext';
...
export function ProjectKanbanBoard({ projectId }: ProjectKanbanBoardProps) {
  ...
  const boardScrollRef = useHorizontalScroll<HTMLDivElement>();
  ...
  const handleDrop = useCallback(
    ...
    [draggingTask, moveTask, currentProjectId]
  );
  ...
  const handleSaveTask = useCallback(
    ...
    [editingTask, addTask, updateTask, currentProjectId]
  );
  ...
  }, [taskToDelete, deleteTask, currentProjectId]);
  ...
  if (!activeProject) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center">
        <Layers className="w-12 h-12 mb-4 text-[hsl(var(--theme-muted-foreground))]" />
        <h2 className="text-xl font-semibold text-[hsl(var(--theme-foreground))] mb-2">
          No Project Selected
        </h2>
        <p className="text-[hsl(var(--theme-muted-foreground))]">
          Select or create a project to start managing tasks
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full" onDragEnd={handleDragEnd}>
      ...
      <div ref={boardScrollRef} className="hidden sm:block flex-1 overflow-x-auto overflow-y-hidden pb-4 scrollbar-hide">
        <motion.div
