...
// Note: Convex queries are disabled until ConvexProvider is set up
...
// Temporary ID type until Convex is configured
...
function getSessionId(): string {
  ...
  let sessionId = localStorage.getItem('jamesOS_session_id');
  if (!sessionId) {
    sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    localStorage.setItem('jamesOS_session_id', sessionId);
  }
  return sessionId;
}
...
export function SuggestionList({ onSuggestClick }: SuggestionListProps) {
  ...
  // Convex queries disabled until ConvexProvider is set up
  ...
  // Use empty array until Convex is configured
  const suggestions: Suggestion[] | undefined = [];
  const sessionUpvotes: SuggestionId[] | undefined = [];
      ...
      {filteredSuggestions.length === 0 && (
        ...
        >
          ...
          <p className="text-sm text-[hsl(var(--theme-muted-foreground))] mb-4">
            Be the first to suggest a feature for JamesOS!
          </p>
          {onSuggestClick && (
            <button
