...
export function KanbanBoard() {
  ...
  const handleMobileColumnPrev = useCallback(() => {
    setMobileColumnIndex(Math.max(0, mobileColumnIndex - 1));
  }, [mobileColumnIndex]);

  const handleMobileColumnNext = useCallback(() => {
    setMobileColumnIndex(Math.min(COLUMNS.length - 1, mobileColumnIndex + 1));
  }, [mobileColumnIndex]);

  const handleMobileColumnSelect = useCallback((index: number) => {
