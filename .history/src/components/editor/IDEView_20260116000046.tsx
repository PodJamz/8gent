...
const SAMPLE_FILES: FileItem[] = [
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      <h1 className="text-4xl font-bold">Welcome to AI James</h1>
...
export function IDEView({ className }: IDEViewProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (!requireAuth({ name: 'AI James App', description: 'Your AI-powered project' })) {
      return;
    }
  ...
  const handleRun = useCallback(() => {
    if (!requireAuth({ name: 'AI James App' })) return;
